/// System prompt construction, matching Python's agent/prompts.py.

const SYSTEM_PROMPT_BASE: &str = r#"You are OpenPlanter, an analysis and investigation agent operating through a terminal session.
...
const ACCEPTANCE_CRITERIA_SECTION: &str = r#"
== ACCEPTANCE CRITERIA ==
...
const SESSION_LOGS_SECTION: &str = r#"
...
/// Build the system prompt with optional sections based on feature flags.
...
mod tests {
    ...
    fn test_build_system_prompt_base() {
        ...
        assert!(prompt.contains("OpenPlanter"));
        assert!(prompt.contains("EPISTEMIC DISCIPLINE"));
        assert!(prompt.contains("SESSION LOGS"));
    ...
    #[test]
    fn test_build_system_prompt_recursive() {
    ...
    #[test]
    fn test_build_system_prompt_acceptance() {
        let prompt = build_system_prompt(true, true, false);
        assert!(prompt.contains("ACCEPTANCE CRITERIA"));
        assert!(prompt.contains("VERIFICATION PRINCIPLE"));
    }

    #[test]
    fn test_build_system_prompt_demo() {
        let prompt = build_system_prompt(false, false, true);
        assert!(prompt.contains("Demo Mode (ACTIVE)"));
    }

    #[test]
    fn test_build_system_prompt_all() {
        ...
        let ac_pos = prompt.find("ACCEPTANCE CRITERIA").unwrap();
        let demo_pos = prompt.find("Demo Mode").unwrap();
        assert!(repl_pos < ac_pos);
        assert!(ac_pos < demo_pos);
    }
}
